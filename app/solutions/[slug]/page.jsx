import { SolutionEvidence } from "../../components/portfolio-evidence";
import { SolutionCollaboration } from "../../components/service-contribution";
import { notFound, permanentRedirect } from "next/navigation";
import Link from "../../components/site-link";
import { solutions, services, enquiry } from "@/data/catalogue";
import { SectionTitle, CTA, Arrow } from "../../components/connected";
const aliases = {
...
export default async function Solution({ params }) {
  const { slug } = await params;
  if (aliases[slug]) permanentRedirect("/solutions/" + aliases[slug]);
  const s = solutions.find((s) => s.id === slug);
  if (!s) notFound();
